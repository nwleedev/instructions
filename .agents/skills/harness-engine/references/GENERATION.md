...
도메인 조사 결과를 바탕으로 현재 프로젝트의 아키텍처를 해당 도메인의 권장 패턴 대비 평가한다.

절차:

1. 도메인 조사에서 확인한 권장 아키텍처 패턴을 정리한다.
   - 공식 문서, 표준, 검증된 OSS 프로젝트를 패턴 출처로 사용한다.
   - 어댑터에 아키텍처 스타일 분기가 정의되어 있으면 (예: FSD vs Layered) contract packet에서 확정된 스타일을 평가 기준으로 삼는다.
   - 어댑터가 없으면 (engine-asset bootstrap 경로) 도메인 조사와 contract packet만으로 평가 기준을 수립한다.

2. 현재 프로젝트의 아키텍처를 각 권장 패턴에 대해 분류한다.
...
- 프로젝트의 현재 구조를 맹목적으로 수용하지 않는다. 아키텍처 평가 없이 현 상태를 곧 권장으로 서술하는 것은 금지한다.
...
- 조사 중간 산출물, 임시 메모, 캐시 등 임시 파일은 반드시 `session_path` 아래 `temps/` 폴더에 기록한다.
- 시스템 임시 디렉터리(`/tmp` 등)를 사용하지 않는다.
- 세션이 없는 환경에서는 프로젝트 루트의 `temps/`를 사용한다.
- 상세 규칙은 `instructions/SESSIONS.md`의 "임시 파일" 섹션을 따른다.
