...
- 테스트 전용 문서
- 도메인별 세부 문서
- `enforcement/*.md` — 강제 규칙 설정 가이드 (lint, 정적 분석, CI 등). 소프트웨어 도메인에서 스택이 감지된 경우에 추가한다.
...
- `ARCHITECTURE.md`
  - 소프트웨어 도메인: 작업 흐름, 계층, 역할 분리, 세션/도구 사용 방식, 설계 근거
  - 비소프트웨어 도메인: 작업 구조, 방법론, 절차 흐름, 도구/자원 사용 방식, 설계 근거
  - 현재 프로젝트 구조/방법론 평가 (도메인 권장 패턴 대비 충족/부분충족/미충족/해당없음 분류, 의도적 일탈 기록)
- `ANTI_PATTERNS.md`
  - 피해야 할 구현/조사/운영 패턴과 차단 규칙, 근거 출처
- `VALIDATION.md`
  - 완료 기준, 드라이런, 검증 체크리스트, 검증 근거
- `enforcement/*.md` (소프트웨어 도메인, 선택)
  - ANTI_PATTERNS.md의 안티패턴 중 자동 검출 가능한 항목을 린트/정적 분석 규칙으로 매핑
  - 각 규칙에 대해: 규칙 이름, 심각도(error/warn), 적용 근거, 설정 코드 블록을 포함
  - 실제 설정 파일(.eslintrc, ruff.toml 등)은 이 문서를 기반으로 구현 시 생성한다. 하네스 생성 시점에는 가이드 문서만 산출한다.
