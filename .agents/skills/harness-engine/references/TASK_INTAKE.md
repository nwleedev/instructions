...
- 시장 조사/경쟁 분석/제품 기획 요청 → `instructions/research` (제품 기획 조사 섹션)
...
- **어댑터 없음 + 대표 분류**: 일반 하네스 생성으로 보내지 않는다. `engine-asset bootstrap` 경로로 전환하고, 해당 도메인의 `instructions/<task_type>` 하네스와 thin adapter, 필요 시 example pack과 stack seed 문서까지 한 세트로 생성한다.
...
- packet 경로: `store/<session_id>/temps/contracts/<task_type>-contract.md` 또는 `temps/contracts/<task_type>-contract.md`
- packet에 들어갈 프로젝트 스택
- packet에 들어갈 필수 작업 축
- packet에 들어갈 금지 패턴
