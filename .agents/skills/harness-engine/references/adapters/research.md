...
harness-engine이 조사 도메인 하네스를 생성하거나 보강할 때, 공통 `research` phase 이후 이 어댑터를 추가 적용하여 조사 task_type 고유의 Coverage Contract, Anti/Good 필수 쌍, 검증 기준을 적용한다.

공통 `research` phase는 모든 task_type에 먼저 적용된다. 이 문서는 최종 `task_type`이 `research`일 때 필요한 추가 계약이다.
...
조사 task adapter는 아래 example pack과 함께 유지한다.
...
- 공통 `research` phase가 기본 조사 품질을 닫고, 이 adapter는 `research` task_type의 최종 산출물 요구를 닫는다.
