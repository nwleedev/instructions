...
harness-engine이 프론트엔드 도메인 하네스를 생성하거나 보강할 때, 이 어댑터를 로드하여 프론트엔드 고유의 Coverage Contract, Anti/Good 필수 쌍, 검증 기준을 적용한다.
...
- `references/examples/frontend/*`에는 현재 저장소의 강한 프론트엔드 하네스에서 추린 reference-only evidence가 있다.
...
- Coverage Contract 필수 축: 기존 instructions/frontend 하네스 (INDEX.md, ARCHITECTURE.md, ANTI_PATTERNS.md) 분석 기반
