...
각 목표나 추가 작업마다 해당 작업을 성공적으로 완료하기 위해서 추가적으로 필요한 부분이 있는지 조사해주시고 사용자에게 선택지를 제시해주세요.

만약 사용자가 추가 정보를 제공해도 누락된 항목을 또 발견하면 다시 사용자에게 선택지를 제시해주세요.

- 누락된 항목이 사라질 때까지 선택지를 반복적으로 제시합니다. 모든 정보가 갖춰지면 진행.

PLANS.md, 지침, 하네스 환경으로 작업을 완료할 수 없다고 판단할 때마다 필요한 정보를 조사해서 사용자에게 선택지를 제시합니다.
