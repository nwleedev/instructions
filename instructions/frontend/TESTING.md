...
이 문서는 React 기반 프론트엔드 작업에서 Codex, Claude가 테스트를 **구현 완료 후 확인 절차**가 아니라 **구현을 이끄는 실행 명세**로 다루도록 만들기 위한 기본 작업 프로토콜이다.
...
2. 테스트 종류 선택 기준
3. 완료 판단 기준과 증빙 형식
4. ESLint, 테스트 러너, CI, Codex, Claude 작업 로그의 책임 분리
...
이 문서는 Codex, Claude가 테스트 작업을 진행할 때 따라야 하는 **프로토콜 문서**다.
...
### 8.4 Codex, Claude 작업 로그가 맡는 것

Codex, Claude 작업 로그 또는 PR 기록은 **사람이 검토 가능한 TDD 증빙**을 남긴다.
