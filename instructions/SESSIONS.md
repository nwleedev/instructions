...
- `./store/<session_id>/PLANS.md` 파일
  ...
  - 장시간 작업에서도 기준 문서로 유지하며, Codex는 필요한 정보만 **append-only** 방식으로 추가할 수 있다.
  - 만약 작업을 진행하기 위해 기존 PLANS.md 파일에서 제거할 내용이 있을 경우 사용자에게 제거요청을 한다.

### `PLANS.md` 갱신 방식

- `PLANS.md`의 목표, 범위, 완료 기준, 금지 사항은 사용자 결정 문맥으로 취급한다.
- Codex는 `PLANS.md`를 읽고 부족하거나 충돌하는 항목을 식별할 수 있다.
- 작업 완료에 필요한 정보가 비어 있거나 모호하면, Codex는 `PLANS.md`에 어떤 정보가 추가로 필요한지 사용자에게 제시한다.
- 사용자가 선택지나 답변으로 정보를 전달하면, Codex는 그 내용을 기존 `PLANS.md`에 **추가하는 방식으로만** 반영한다.
- Codex는 `PLANS.md`의 기존 내용을 변경하거나 삭제하지 않는다.
- 새 정보가 기존 `PLANS.md`와 충돌해 기존 내용을 바꿔야 하는 경우에는 Codex가 임의로 수정하지 않고 사용자에게 검토를 요청한다.
...
장시간 작업 또는 다단계 작업의 경우 다음 섹션을 유지한다. Codex가 이를 보강할 때도 기존 내용을 바꾸지 않고 필요한 정보만 뒤에 추가한다.
...
- 필수 항목이 비어 있으면 Codex는 질문을 흩어 묻지 않고 한 번에 묶어서 제시한다.
...
- 질문은 반드시 **선택지 형태**로 제시하며, 각 선택지마다 다음을 포함한다.
  ...
  - 단점
  - 잠재적인 리스크
- 사용자가 선택지에 답한 뒤에는 Codex가 그 답을 기존 `PLANS.md`에 추가하는 방식으로 반영한다.
...
- 위 정보가 세션 파일에 기록되어 있다면, Codex는 사용자에게 같은 배경 설명을 다시 요구하지 않고 이어서 진행한다.
...
- Codex CLI 및 사람(리뷰어, 관리자)이 프로젝트 흐름을 빠르게 파악하는 용도이다.
- 규칙
  - 다음 형식을 기본으로 유지한다:
...
- Codex CLI가 작업 이해도를 높이기 위해 참고해야 하는 문서이다.
