...
2. 프로젝트 로컬 작업 분야 하네스 확인
   - 현재 프로젝트에 `instructions/<task_type>/INDEX.md` 같은 로컬 작업 분야 하네스가 있으면 그것을 읽는다.
...
- 이 저장소에 있는 `instructions/frontend/*`, `instructions/research/*`, `instructions/learning-mode/*`는 현재 저장소에서 쓰는 repo-local 상세 하네스다.
- 위 상세 하네스들은 portable core가 아니며, 다른 프로젝트의 기본 복사 범위로 가정하지 않는다.
- 다른 프로젝트에서는 해당 프로젝트에 맞는 로컬 작업 분야 하네스를 따로 만들거나 `harness-engine`으로 생성한다.
